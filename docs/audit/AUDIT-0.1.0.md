...
| **L12** | Optional CSPRNG interleaver has no ergonomic vault-wiring path | `src/fec/mod.rs`, `src/vault.rs` | Add a documented constructor that wires the HKDF interleaver seed into a `BlockThenCsprng` `ConcatenatedFec` from a master. | **FIXED** (0.2.0) — `ConcatenatedFec::with_csprng_from_master(master, depth)` HKDF-derives the seed and builds `BlockThenCsprng`; documented opt-in/non-security, unit-tested via `CryptoVault::new`. **Verification-pass hardening:** the constructor now validates `master.len() == KEY_LEN` up front (`InvalidInput` otherwise), mirroring the M1 byte-core guard so a wrong-length master is never silently HKDF-expanded. |
...
| ID | Title | Fix | Status |
|----|-------|-----|--------|
| **N1** | File headers say `// Version: 2.0.0` while the crate is 0.1.0 | Set headers to the crate version. | **FIXED** (0.2.0) — all `src/**/*.rs` headers set to `0.2.0`. **Verification-pass scope closure:** the `tests/*.rs` file headers (which still read `2.0.0`/`1.0.0`) are now also aligned to `0.2.0`, matching the crate version across the whole package. |
| **N2** | Two decrypt-path messages interpolate the attacker's own input length (b64 len, base64 crate error, unreachable guard) | Harmonize to fixed generic strings for full SR-R7 message consistency (b64-length echo + unreachable `body.len()` guard fixed; base64-crate `Encoding` error left as-is). | **FIXED** — **Verification-pass closure:** the remaining base64-crate `Encoding` path is now sanitized too, mapping `STANDARD.decode` errors to a fixed generic `"invalid encoding"` message (drops the crate's interpolated byte-index/offset); the SR-R7 no-structural-detail test now also covers malformed base64 (bad alphabet/padding/trailing bits). |
...
| C (CI/docs) | **M4** (doctest step in both workflows), **L5** (MSRV 1.96 job), **M3-docs** (decrypt-path CPU-DoS docs), **L11** (extended RS/Viterbi KAT corpus), **L12** (`with_csprng_from_master` helper), **L13** (docs.rs link repoint), **N3** (README numbers → qualitative + badge), **N4** (direct-codec no-cap notes) |
...
- **M6** — genuinely independent (non-owner) external review of the `reedsolomon`/`viterbi` FEC crates, required before claiming SR-F5. Recorded under the gate's waiver path. **Non-blocking; accepted follow-up** — per project decision the independent external FEC review is explicitly **not** a current release objective; the owner-sign-off waiver is the accepted position for 0.x.
