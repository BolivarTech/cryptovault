# External FEC Review — `cryptovault` 0.3.0 (SR-F5)

> **Purpose.** `cryptovault` and its FEC crates (`reedsolomon` 0.2.0,
> `viterbi` 0.2.0) share a single author. Self-consistency is **not** correctness:
...
>
> **Status: SIGNED — owner sign-off recorded.** This artifact documents the
> technical basis (the in-repo independent references, KATs, correction-capacity
> and no-panic evidence) on which the signer attests. The `xtask review-gate`
> command requires this file to carry a non-empty reviewer-identity line and a
> non-empty review-date line; both are completed in the **Sign-off** section
> below, so the release gate for a `v0.3.0` tag passes.
>
> **NOT waivable.** There is no `--waive` flag: a silent escape hatch would defeat
> the very blind-spot this review exists to mitigate. A genuine waiver requires a
> documented **owner sign-off + rationale recorded in the release notes**, never a
> CLI toggle.

*Prepared and signed: 2026-07-07. Version gated: 0.3.0 (matches root `Cargo.toml`).*

---

## What changed in 0.3.0 (FEC-review relevance)

**No FEC-crate change.** v0.3.0 adds a **first-class `ViterbiOnlyFec`
`ErrorCorrection` strategy** (AEAD + Viterbi-only; no Reed-Solomon, no
interleaver) as a thin wrapper in `cryptovault` itself (`src/fec/viterbi.rs`) — it
delegates encode/decode to the **unchanged** `ViterbiCodec`/`viterbi 0.2.0` and
reuses the existing `rs_len_from_body` framing helper. The pinned FEC crates
(`reedsolomon` `=0.2.0`, `viterbi` `=0.2.0`) are **identical to 0.2.2**, so the FEC
encode/decode paths under review are unchanged and the 0.2.2 review evidence below
**carries forward verbatim**. The new wrapper's own no-panic / framing / round-trip
behavior is covered by unit + integration (`tests/viterbi_only_fec.rs`, SC-1/2/3/8
+ framing KAT) tests and a dedicated `cargo-fuzz` target
(`fuzz/fuzz_targets/decrypt_viterbi_only.rs`, 210k-run local smoke, zero crashes).

## Scope

- **Crates under review:** `reedsolomon` 0.2.0 (outer RS(255,223)),
  `viterbi` 0.2.0 (inner K=7, R=1/2 convolutional / Viterbi).
- **Version gated:** `0.3.0` — matches the root `Cargo.toml` `version`.
...
## Reed-Solomon `reedsolomon` 0.2.0 — correction capacity & boundaries
...
## Viterbi `viterbi` 0.2.0 — termination & boundaries
...
- [x] **Zero-tail termination:** the `m = 6` flush-bit tail and the `2L + 2`
      ...
      against the real `viterbi` 0.2.0 encoder in
      `test_p0_3_viterbi_termination_overhead_is_two_bytes_per_block`
      (`coded.nbits == (8L + 6) * 2`, `coded.bytes.len() == 2L + TERMINATION_OVERHEAD`,
      `TERMINATION_OVERHEAD == 2`). This drives `MAX_BLOB_LEN`. **Also re-exercised
      by the 0.3.0 `ViterbiOnlyFec` framing KAT** (`tests/viterbi_only_fec.rs`,
      `test_kat_viterbi_only_framing_length_relation`: `2L + 2` single-chunk,
      `2L + 2·2` two-chunk).
...
- [x] **Misdecode backstop:** a Viterbi misdecode — even a silent one beyond KAT
      ...
      `tests/scenarios.rs` corruption-beyond-capacity; and, for the Viterbi-only
      strategy, `tests/viterbi_only_fec.rs` SC-3; gate 12 of `docs/no-panic.md`).

## No-panic robustness (SR-R5)

- [x] Every FEC-crate entry point on the decrypt path is enumerated and gated by
      structural validation — the entry-point → gate table in
      ...
      The 0.3.0 `ViterbiOnlyFec::validate_pre_fec` applies the same `MAX_BLOB_LEN`
      cap + chunked-Viterbi framing gate before any decode.
- [x] `cargo-fuzz` targets (`decrypt`, `unwrap`, `fec_crates`, **`decrypt_viterbi_only`**)
      show no reachable panic in the local smoke run (zero crashes). The
      `fec_crates` target drives `reedsolomon` / `viterbi` **directly, unguarded**;
      the new `decrypt_viterbi_only` target drives the AEAD + Viterbi-only decrypt
      path (210k runs, 0 crashes).
...
| — | — | — | No correctness defect found; FEC crates unchanged from 0.2.2. | — |
...
> waiver path for a single-author project (an unenforced gate is no gate; a waiver
> requires a documented owner sign-off + rationale). The FEC crates
> (`reedsolomon` 0.2.0, `viterbi` 0.2.0) are **unchanged from 0.2.2**, so the 0.2.2
> owner sign-off carries forward on the technical merits. A fully-independent
> third-party review of the FEC crates remains a recorded, non-blocking follow-up.

- **Reviewer:** Julian Bolivar (BolivarTech)
- **Affiliation / independence statement:** Author and owner of `cryptovault` and
  of the reviewed FEC crates (`reedsolomon` 0.2.0, `viterbi` 0.2.0). This is an
  **OWNER sign-off**, not an independent third-party review — recorded as the
  gate's sanctioned single-author waiver, with the technical basis documented
  above. An independent FEC-crate review remains a tracked, non-blocking follow-up.
- **Date:** 2026-07-07
- **Verdict:** APPROVE for v0.3.0. The FEC encode/decode paths are **unchanged
  from 0.2.2** (pinned `reedsolomon` 0.2.0 / `viterbi` 0.2.0) and were validated
  ...
  the chunked-Viterbi boundary math, and the no-panic audit. The new
  `ViterbiOnlyFec` wrapper adds no FEC-crate code and is covered by its own
  round-trip / noisy-channel / boundary / fuzz tests. The AEAD is the sole
  integrity anchor; the FEC is resilience only, never security.
- **Notes:** Owner sign-off recorded under explicit owner authorization. The
  pinned FEC crates (`reedsolomon` `=0.2.0`, `viterbi` `=0.2.0`) are **unchanged**
  from 0.2.2, so this FEC review carries forward verbatim; v0.3.0 adds only the
  `ViterbiOnlyFec` wrapper in `cryptovault` (no FEC-crate code change). The
  independent (non-owner) FEC-crate review remains a documented, non-blocking
  follow-up.
