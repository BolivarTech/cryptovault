...
> `viterbi` 0.1.0) share a single author. Self-consistency is **not** correctness:
...
> left blank here, so the release gate for a `v0.2.1` tag will **fail** until a
...
*Draft prepared: 2026-07-04. Version gated: 0.2.1 (matches root `Cargo.toml`).*
...
- **Crates under review:** `reedsolomon` 0.2.0 (outer RS(255,223)),
  `viterbi` 0.1.0 (inner K=7, R=1/2 convolutional / Viterbi).
- **Version gated:** `0.2.1` — matches the root `Cargo.toml` `version`.
...
## Viterbi `viterbi` 0.1.0 — termination & boundaries
...
- [x] **Zero-tail termination:** the `m = 6` flush-bit tail and the `2L + 2`
      ...
      against the real `viterbi` 0.1.0 encoder in
...
> third-party review of the FEC crates is a recorded post-0.2.1 follow-up.

- **Reviewer:** Julian Bolivar (BolivarTech)
- **Affiliation / independence statement:** Author and owner of `cryptovault`
  and of the reviewed FEC crates (`reedsolomon` 0.2.0, `viterbi` 0.1.0). This is
  ...
  above. An independent FEC-crate review is tracked as a post-0.2.1 follow-up.
- **Date:** 2026-07-04
- **Verdict:** APPROVE for v0.2.1. The FEC encode/decode paths were validated
...
- **Notes:** Owner sign-off recorded under explicit owner authorization.
  `cryptovault` 0.2.1 is the audit-remediated release (see
  [`docs/audit/AUDIT-0.1.0.md`](../audit/AUDIT-0.1.0.md)); the pinned FEC crates
  (`reedsolomon` `=0.1.0`, `viterbi` `=0.0.1`) are **unchanged** from 0.1.0, so
  this FEC review carries forward verbatim. The FEC codecs were bumped for 0.2.1 (`reedsolomon` 0.1.0→0.2.0, `viterbi`
  0.0.1→0.1.0) and verified **wire-identical** to 0.2.0 via the reference KATs
  (RS(255,223) parity, CCSDS Viterbi impulse) — behavior-preserving, no blob-format
  change. The independent (non-owner) FEC-crate review remains a documented,
  non-blocking follow-up condition.
