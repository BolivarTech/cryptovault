...
> left blank here, so the release gate for a `v0.2.2` tag will **fail** until a
...
*Draft prepared: 2026-07-05. Version gated: 0.2.2 (matches root `Cargo.toml`).*
...
- **Version gated:** `0.2.2` — matches the root `Cargo.toml` `version`.
...
> third-party review of the FEC crates is a recorded post-0.2.2 follow-up.

- **Reviewer:** Julian Bolivar (BolivarTech)
- **Affiliation / independence statement:** Author and owner of `cryptovault`
  and of the reviewed FEC crates (`reedsolomon` 0.2.0, `viterbi` 0.2.0). This is
  ...
  above. An independent FEC-crate review is tracked as a post-0.2.2 follow-up.
- **Date:** 2026-07-05
- **Verdict:** APPROVE for v0.2.2. The FEC encode/decode paths were validated
...
- **Notes:** Owner sign-off recorded under explicit owner authorization.
  `cryptovault` 0.2.2 is the audit-remediated release (see
  ...
  this FEC review carries forward verbatim. The `viterbi` codec was bumped for 0.2.2 (0.1.0→0.2.0) and verified
  **wire-identical** via the reference KATs (CCSDS K=7 R=1/2 Viterbi impulse) —
  behavior-preserving, no blob-format change (`reedsolomon` stays at 0.2.0). The independent (non-owner) FEC-crate review remains a documented,
